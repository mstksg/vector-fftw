...
complexR2CScaling :: (MS.Storable a, Floating a, Enum a) => a -> Int -> MS.MVector RealWorld (Complex a) -> IO ()
complexR2CScaling !t !n !a = do
    ...
    unsafeModify a 0 $ scaleComplex s1
    if odd n
        then multC scaleComplex s2 (MS.unsafeSlice 1 (len-1) a)
        else do
            unsafeModify a (len-1) $ scaleComplex s1
            multC scaleComplex s2 (MS.unsafeSlice 1 (len-2) a)
...
dct2 = U.dct2 {normalization = \n -> modifyOutput $ \a -> do
    ...
    let !s2 = sqrt $ 1 / (2*n')
    unsafeModify a 0 (*s1)
    multC scaleScalar s2 (MS.unsafeSlice 1 (MS.length a-1) a)
    }
